...
Private Sub Form_Unload(Cancel As Integer)
    On Error Resume Next
    If CExit = True Then
        Dim yc As VbMsgBoxResult
        yc = MsgBox("Are you sure you want to quit?", vbQuestion + vbYesNo, "Exit")
        If yc = vbNo Then
            Cancel = 1
            Exit Sub
        End If
...
Private Sub LVS_DblClick()
    On Error Resume Next
    ShellExecute Me.hwnd, "Open", LVS.SelectedItem.Key, 0&, 0&, SW_SHOWNORMAL
End Sub
...
Private Sub mnuABT_Click()
    ...
    "Version : 1.02", vbInformation, "About"
End Sub
...
Private Sub tmrU_Timer()
    For i = LVL.ListItems.Count To 1 Step -1
        LVL.ListItems.Item(i) = LVL.ListItems.Item(i) - 1
        If LVL.ListItems.Item(i) < 1 Then
            Dim Index As Integer
            Index = CInt(Replace(LVL.ListItems.Item(i).Key, "x", ""))
            LVL.ListItems.Remove CInt(i)
            If Transfer(Index).State <> 7 Then Exit Sub
...
Private Sub Transfer_Close(Index As Integer)
    ...
    If LOFS > TSize(Index) - 1 And TSize(Index) > 0 Then
        ...
        If Right(LCase(LVD.ListItems("x" & Index).SubItems(1)), 3) = "mp3" Then
            MP3FileName = DownloadDir & "\" & LVD.ListItems("x" & Index).SubItems(1)
            ReadHeader CStr(MP3FileName)
            frmMain.LVS.ListItems.Add , MP3FileName, LVD.ListItems("x" & Index).SubItems(1), , 1
            frmMain.LVS.ListItems(MP3FileName).SubItems(1) = Mid(Left(DownloadDir, Len(DownloadDir)), InStrRev(Left(DownloadDir, Len(DownloadDir)), "\") + 1, Len(Left(DownloadDir, Len(DownloadDir))))
            frmMain.LVS.ListItems(MP3FileName).SubItems(2) = MP3HeaderInfo.Duration
            frmMain.LVS.ListItems(MP3FileName).SubItems(3) = MP3HeaderInfo.Bitrate
            frmMain.LVS.ListItems(MP3FileName).Tag = """" & MP3FileName & """ 0 " & (FileLen(MP3FileName) - 1) & " " & MP3HeaderInfo.Bitrate & " " & MP3HeaderInfo.Frequency & " " & MP3HeaderInfo.PlayTime
            If SCD = True Then SendData LVS.ListItems(MP3FileName).Tag, Chr(100) & Chr(0), Int(LVD.ListItems("x" & Index).Tag)
       End If
...
ReCheck1:
        ...
        Case 10
            ...
            DoEvents
                ...
                If NSend(Index) > 1 Then NSend(Index) = NSend(Index) + 1
               ...
               Close #253
                    ...
                    NSend(Index) = NSend(Index) - Len(CStr(hldSize))
...
Private Sub Transfer_SendProgress(Index As Integer, ByVal bytesSent As Long, ByVal bytesRemaining As Long)
    On Error Resume Next
    If TState(Index) = 12 Then NSend(Index) = NSend(Index) + bytesSent
    If bytesRemaining = 0 And TState(Index) = 12 Then
            Open UFile(Index) For Binary Access Read As #253 'use #253 for uploads
                ...
                End If
            Close #253
        Dim NowWait As Integer
        NowWait = 0
        If (NSend(Index) - LSend(Index)) / (tmrSpeed.Interval / 1000) > ((MaxSpeed * 1024) - 1) Then NowWait = 2 ' See if speed is > current speed per sec if it is dont send for one sec.
        If NowWait > 0 Then
            LVL.ListItems.Add , "x" & Index, NowWait
        Else
            Open UFile(Index) For Binary Access Read As #253 'use #253 for uploads
            Dim Sdta As String
            Sdta = Space$(4096)
