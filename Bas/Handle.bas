...
End If
        ...
        Case "95-2" '607 (0x25f) upload request ( someone wants a file from you )
            ...
            If isShare = 0 And OUS = True Then
                For jk = 1 To frmMain.LVU.ListItems.Count 'See if user is d/l from u already
                    If LCase(frmMain.LVU.ListItems.Item(jk)) = LCase(Title) Then
                        isShare = 1
                        Exit For
                    End If
                Next
